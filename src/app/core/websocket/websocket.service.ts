...
export class WebsocketService implements IWebsocketService, OnDestroy {
  ...
  * */
  public send(event: string, id: number, data: any = {}): void {
    if (event && this.isConnected) {
      // @ts-ignore
      this.websocket$.next({
        // {'type': 'websocket.receive', 'text': '"{\\"event\\":\\"message\\",\\"id\\":2,\\"data\\":{\\"message\\":\\"google\\"}}"'}
        // this.websocket$.next(<any>JSON.stringify({ event, id, data }));
        'event': event,
        'id': id,
        'data': data
      });
